import { Linkedin, Users, Mail, PhoneCall, ArrowUpRight, BadgeCheck } from "lucide-react";
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-muted-foreground">
              The site now includes structured mentor names, colleges, companies, domains, and example student leads.
            </p>
          </div>
            ...
            {mentorPasted.map((mentor) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl bg-[linear-gradient(135deg,#5a67cf_0%,#616ed0_45%,#fbacc3_100%)] text-white shadow-[0_18px_28px_-18px_rgba(90,103,207,0.8)]">
                    <span className="text-lg font-black">{mentor.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] font-semibold uppercase tracking-[0.34em] text-primary/60">{mentor.domain}</div>
                    <div className="mt-1 flex items-center gap-2">
                      <h3 className="font-display text-lg font-bold text-primary">{mentor.name}</h3>
                      <BadgeCheck className="h-4 w-4 text-primary" />
                    </div>
                    <div className="mt-1 text-sm text-muted-foreground">{mentor.college}</div>
                    <div className="text-sm text-muted-foreground">{mentor.company}</div>
                  </div>
                </div>

                <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-[#0a66c2]/20 bg-[#0a66c2]/10 px-3 py-1.5 text-xs font-semibold text-[#0a66c2] transition group-hover:bg-[#0a66c2] group-hover:text-white">
          ...
          <div className="space-y-5">
            <div className="rounded-3xl border border-border bg-background p-5">
              ...
              <div className="mt-4 space-y-3">
                {contactLeads.map((lead) => (
                  <div key={lead.email} className="rounded-2xl border border-border bg-card p-4">
            ...
            <div className="rounded-3xl border border-border bg-gradient-to-br from-primary-soft to-background p-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                The site now surfaces the exact group offer language from the pasted notes so students can understand it immediately.
              </p>
            </div>
          </div>
        </div>
