...
import { companyLogos, successStories } from "@/data/programs";
    ...
    <section id="stories" className="bg-primary-soft py-20">
      <div className="container-px mx-auto max-w-7xl">
        <div className="text-center">
          ...
          <h2 className="mt-3 font-display text-3xl font-bold text-primary sm:text-4xl">580+ students. Day-Zero offers.</h2>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {successStories.map((s, index) => {
            ...
            return (
              <ScrollReveal key={s.name} delay={index * 80}>
                <div className="group rounded-[1.6rem] border border-border bg-card p-5 shadow-[var(--shadow-card)] transition hover:-translate-y-1 hover:shadow-[var(--shadow-elegant)]">
                  <div className="flex items-center gap-4">
                    <div className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-gradient-to-br ${gradient}`}>
                      {s.avatar ? <img src={s.avatar} alt={s.name} loading="lazy" className="h-full w-full object-cover" /> : null}
                      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.25),transparent_35%)]" />
                    </div>
          ...
          <div className="mt-4 flex overflow-hidden">
            <div className="marquee flex shrink-0 items-center gap-10 pr-10">
              {[...companyLogos, ...companyLogos].map((c, i) => (
                <span key={i} className="whitespace-nowrap font-display text-lg font-bold text-primary/70">
                  {c}
                </span>
              ))}
            </div>
